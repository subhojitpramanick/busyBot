import { Download, Eraser, Sparkle, Sparkles } from "lucide-react";
...
const RemoveBackground = () => {
  ...
  // --- Mutation for removing image background ---
  const { mutate: removeBackground, isPending: loading } = useMutation({
    mutationFn: async (imageFile) => {
      ...
      return data.content; // Return the new image URL
    },
  ...
  // Handles the form submission to the background removal API
  const onSubmitHandeler = (e) => {
    e.preventDefault();
    if (input) removeBackground(input);
  };
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      link.href = url;
      link.setAttribute("download", `background-removed-${Date.now()}.png`);
    ...
    <div className="h-full overflow-y-scroll p-4 sm:p-8 flex items-start justify-center flex-wrap gap-8 text-slate-700 bg-gradient-to-br from-indigo-100 to-pink-100 min-h-screen">
      ...
      >
        <div className="flex items-center gap-3 mb-2">
          <Eraser className="w-7 h-7 text-orange-500 drop-shadow" />
        ...
        >
          {loading ? (
            ...
            <Eraser className="inline w-5 h-5" />
          )}
          Remove Background
        </button>
      </form>
      ...
      <div className="w-full max-w-lg p-6 bg-white rounded-2xl shadow-lg flex flex-col border border-orange-200 min-h-[400px] max-h-[600px]">
        <div className="flex items-center justify-between gap-3 mb-4">
          <div className="flex items-center gap-3">
            <Eraser className="w-6 h-6 text-orange-500 drop-shadow" />
            <h1 className="text-2xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-orange-500 via-orange-300 to-pink-400">
              Processed Image
            </h1>
          </div>
